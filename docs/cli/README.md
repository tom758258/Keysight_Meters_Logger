...
tracked project file. If it is missing, first rerun the root [README Install](../../README.md#install)
flow with `uv sync --all-extras --link-mode=copy`; the optional editable refresh
described above is also available for an existing virtual environment. If
PowerShell blocks activation because of execution policy, keep using the
...
- If `.\.venv\Scripts\meters-tool.exe` is missing, first rerun the root
  [README Install](../../README.md#install) flow with
  `uv sync --all-extras --link-mode=copy`. For an existing virtual environment,
  the optional editable refresh described in Development is also available.
...
flow before running tests. For an existing virtual environment, the optional
editable refresh described in Development is also available.
